...
### 等級系統（XP）
- 星星＝經驗值。累積星星越多，等級越高（`RANKS` 表，共 **50 級**，一路衝到 **40,800★**，
  讓長期都有下一級可追；星星門檻採**遞增間隔**，越後面每級要越多星）。
- 標題列有即時 **XP 進度條**，顯示「再 N ⭐ 升到下一級」；達 Lv50 顯示「已達最高等級」。
- 升級時跳**慶祝彈窗 + 彩帶**（`toast()` + `confetti()`）。
- 稱號主題隨等級升級：小動物 → 勇者/王者 → 魔法 → 猛獸 → 星際 → 神話。

**里程碑（完整 50 級見 `progress.js` 的 `RANKS`）**：

| Lv | 稱號 | 星星 | | Lv | 稱號 | 星星 |
|----|------|------|---|----|------|------|
| 1 | 🥚 英文小蛋 | 0 | | 25 | 🦁 百獸之王 | 9,200 |
| 9 | 🏆 傳說英雄 | 600 | | 30 | 🦚 輝煌之翼 | 13,600 |
| 16 | 💎 鑽石英雄 | 3,600 | | 40 | 🌠 流星傳說 | 25,300 |
| 17 | 🛡️ 鋼鐵守護者 | 4,050 | | 50 | 🥇 英文之神 | 40,800 |

> 前 16 級門檻與舊版**完全一致**，只往上延伸 17–50 級，因此既有存檔的等級不會位移。
